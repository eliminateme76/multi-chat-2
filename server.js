...
async function getState() {
  const project = (await pool.query('SELECT id,title,location,mood,scene_time AS time,description,rules,scene_number AS "sceneNumber",turn_number AS turn,director_note AS "directorNote" FROM projects WHERE id=$1', [PROJECT_ID])).rows[0];
  if (!project) return null;
  const [characters, relationships, entries] = await Promise.all([
    pool.query('SELECT id,name,role,emoji,color,personality,speech_style AS "speechStyle",goal,secret,emotion FROM characters WHERE project_id=$1 ORDER BY sort_order', [PROJECT_ID]),
    pool.query('SELECT from_character_id AS "from",to_character_id AS "to",label,score FROM relationships WHERE project_id=$1 ORDER BY created_at', [PROJECT_ID]),
    pool.query('SELECT entry_type AS type,character_id AS "characterId",dialogue AS text,action,event_text AS "eventText" FROM scene_entries WHERE project_id=$1 ORDER BY sort_order', [PROJECT_ID])
  ]);
...
app.post('/api/turns', async (_req, res, next) => { let client; try { const state = await getState(); const character = state.characters[state.turn % state.characters.length]; if (!character) throw new Error('At least one character is required.'); const turn = await generateCodexTurn({ character, state }); client = await pool.connect(); await client.query('BEGIN'); await client.query('INSERT INTO scene_entries (id,project_id,entry_type,character_id,dialogue,action,sort_order) VALUES ($1,$2,$3,$4,$5,$6,$7)', [randomUUID(), PROJECT_ID, 'message', character.id, turn.dialogue, turn.action, await nextOrder(client)]); await client.query('UPDATE characters SET emotion=$2,updated_at=NOW() WHERE id=$1', [character.id, turn.emotion]); await client.query('UPDATE projects SET turn_number=turn_number+1,director_note=$2,updated_at=NOW() WHERE id=$1', [PROJECT_ID, `${character.name} Agent가 Codex로 응답했습니다. 다음 Agent에는 공개된 대화만 전달하세요.`]); await client.query('COMMIT'); res.json(await getState()); } catch (error) { if (client) await client.query('ROLLBACK'); next(error); } finally { client?.release(); } });
