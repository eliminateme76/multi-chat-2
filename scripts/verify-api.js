...
try {
  ...
  if (!['WORLD_ONLY','SCENE','ARC'].includes(operation.result?.narrativeImpact)) throw new Error('Narrative promotion decision is missing from the progression result.');
  if (!operation.result.narrativeImpacts || !['WORLD_ONLY','SCENE','ARC'].includes(operation.result.narrativeImpacts.afterCharacter)) throw new Error('Concordia pre/post narrative promotion decisions are not separated.');
  if (operation.result.narrativeImpact !== 'WORLD_ONLY' && (afterTurn.storyState.rhythm.phase !== operation.result.worldPhase || afterTurn.storyState.rhythm.lastOutcome !== operation.result.worldOutcome)) throw new Error('Persisted story rhythm does not match the promoted World Director resolution.');
