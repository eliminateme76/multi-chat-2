...
async function applyPostCharacterJudgment(client, projectId, operationId, plan, characterOutcome) {
  ...
  try {
    ...
    const judgment = {
      ...
      runtime: { timeToFirstTokenMs: plan.timeToFirstTokenMs, contextTokens: Number(plan.threadUsage?.last?.inputTokens || 0), outputTokens: Number(plan.threadUsage?.last?.outputTokens || 0), promptMode: plan.promptMode, promptCharacters: plan.promptCharacters, threadRolledOver: plan.threadRolledOver },
      engine: engineInfo
    };
...
async function runProgression(pool, projectId, operationId) {
  ...
  try {
    ...
    for (const step of steps) {
      ...
      const storedRuntime = { characterId: step.characterId, timeToFirstTokenMs: turn.timeToFirstTokenMs, contextTokens: Number(turn.threadUsage?.last?.inputTokens || 0), outputTokens: Number(turn.threadUsage?.last?.outputTokens || 0), promptMode: turn.promptMode, promptCharacters: turn.promptCharacters, threadRolledOver: turn.threadRolledOver };
      await client.query(`UPDATE world_operations SET payload=payload || $2::jsonb WHERE id=$1`, [operationId, JSON.stringify({
